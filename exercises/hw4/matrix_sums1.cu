...
// matrix row-sum kernel, DSIZE workers -> each worker sums one row
...
// matrix column-sum kernel, DSIZE workers -> each worker sums one column
__global__ void column_sums(const float *A, float *sums, size_t ds){
  ...
  if (idx < ds){
    float sum = 0.0f;
    for (size_t i = 0; i < ds; i++)
      sum += A[i * ds + idx]; // write a for loop that will cause the thread to iterate down a column, keeping a running sum, and write the result to sums
    sums[idx] = sum;
}}
